...
internal static unsafe class KingMovement
{
        ...
        int offset = 0)
    {
        ...
        ulong pieceBB = position->pieceBBs[Position.BBIndex(Pieces.King, color)];
        ...
        while (BitboardOps.IsNotEmpty(pieceBB))
        {
            ...
            ulong movesBB = ~myBlockers & KingAttacks.AttackMasks[srcSquare];
            ...
            while (BitboardOps.IsNotEmpty(captures))
            {
                ...
                *cursor++ = BinaryMoveOps.EncodeMove(
                    srcSquare, Pieces.King, color, trgSquare, 
                    capture: 1);
