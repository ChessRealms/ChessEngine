...
internal static unsafe class RookMovement
{
        ...
        int offset = 0)
    {
        ...
        ulong pieceBB = position->pieceBBs[Position.BBIndex(Pieces.Rook, color)];
        ...
        while (BitboardOps.IsNotEmpty(pieceBB))
        {
            ...
            ulong movesBB = ~myBlockers & RookAttacks.GetSliderAttack(srcSquare, allBlockers);
            ...
            while (BitboardOps.IsNotEmpty(captures))
            {
                ...
                *cursor++ = BinaryMoveOps.EncodeMove(
                    srcSquare, Pieces.Rook, color, trgSquare, 
                    capture: 1);
